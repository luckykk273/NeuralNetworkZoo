...
def demo():
    ...
    train_imgs = mnist.train.images.reshape([-1, time_steps, num_input])
    train_labels = mnist.train.labels
    test_imgs = mnist.test.images.reshape([-1, time_steps, num_input])
    ...
    # Build model: Single-layer RNN
    # Define a RNN cell
    rnn_cell = tf.compat.v1.nn.rnn_cell.BasicRNNCell(hidden_units)
    # defining initial state
    initial_state = rnn_cell.zero_state(batch_size, dtype=tf.float32)
    # Every input will be called 28 times
    outputs, state = tf.compat.v1.nn.dynamic_rnn(rnn_cell, x, initial_state=initial_state, dtype=tf.float32)

    '''
    In this case, we easily use a dense layer to fit the final output shape.
    (Because we only want to practice how to build a RNN)
    Also can set the last RNN cell units equal to number of classes,
    then we don't have to add another dense layer after RNN.
    '''
