...
def demo():
    ...
    # Build model: Multi-layer RNN
    # Define a RNN cell list
    rnn_layers = [tf.compat.v1.nn.rnn_cell.BasicRNNCell(s) for s in size]

    # create a RNN cell composed sequentially of a number of RNNCells
    multi_rnn_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell(rnn_layers)

    # Every input will be called 28 times
    outputs, state = tf.compat.v1.nn.dynamic_rnn(multi_rnn_cell, x, dtype=tf.float32)
    ...
    '''
    # The shape has to fit the number of the last RNN cell units(that is size[-1])
