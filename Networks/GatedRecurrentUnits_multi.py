...
def demo():
    ...
    # Build model: Multi-layer GRU
    # Define a GRU cell list
    gru_layers = [tf.compat.v1.nn.rnn_cell.GRUCell(s) for s in size]
    # create a GRU cell composed sequentially of a number of GRUCells
    multi_gru_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell(gru_layers)

    # Every input will be called 28 times
    outputs, state = tf.compat.v1.nn.dynamic_rnn(multi_gru_cell, x, dtype=tf.float32)
    ...
    '''
    # The shape has to fit the number of the last GRU cell units(that is size[-1])
