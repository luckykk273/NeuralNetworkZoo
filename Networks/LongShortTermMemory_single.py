...
def demo():
    ...
    # Build model: Single-layer LSTM
    # Define a LSTM cell
    lstm_cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(hidden_units)
    # defining initial state
    initial_state = lstm_cell.zero_state(batch_size, dtype=tf.float32)
    # Every input will be called 28 times
    outputs, state = tf.compat.v1.nn.dynamic_rnn(lstm_cell, x, initial_state=None, dtype=tf.float32)
